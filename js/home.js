...
//     url = 'http://localhost/stereotribes/base/get';
    url = url+'/site/gethome';
    var qstring  = '/width/'+$(window).width();;
    $.get( url+qstring, function( data ) {  $( "#main_grid_content" ).append( data ); init(); $('.freewall .brick').css('visibility', 'visible');$('.brick .mask').css('display', 'block');});

//
// $('#load_more_blocks').click(function(){
//        var page_no = $('#page_number').val();
//        var qstring  = '?page='+page_no;
//        $.get( url+qstring, function( data ) {  $( "#main_grid_content" ).append( data ); init(); $('.freewall .brick').css('visibility', 'visible');$('.brick .mask').css('display', 'block'); });
